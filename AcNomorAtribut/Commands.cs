...
namespace AcNomorAtribut
{
    public class Commands
    {
        ...
        public List<string> GetModelSpaceBlockReferenceOIDCByBlockName(string Filename, out List<string> arrList)
        {
            ObjectIdCollection returnOIDs = null;
            arrList = new List<string>();
            Database dbFromFile = new Database(false, true);
            try
            {
                dbFromFile.ReadDwgFile(Filename, System.IO.FileShare.Read, false, "");
            }
            catch (System.Exception ex)
            {
                AcAp.ShowAlertDialog(ex.Message);
            }
            using (DocumentLock docloc = AcAp.DocumentManager.MdiActiveDocument.LockDocument())
            {
                using (dbFromFile)
                {
                    using (Transaction trans = dbFromFile.TransactionManager.StartTransaction())
                    {
                        BlockTable bt = (BlockTable)trans.GetObject(dbFromFile.BlockTableId, OpenMode.ForRead);
                        BlockTableRecord btrModel = (BlockTableRecord)trans.GetObject(bt[BlockTableRecord.ModelSpace], OpenMode.ForRead);
                        returnOIDs = new ObjectIdCollection();
                        foreach (ObjectId thisOid in btrModel)
                        {
                            Entity blk = (Entity)trans.GetObject(thisOid, OpenMode.ForRead);
                            if (blk.GetType().ToString() == "Autodesk.AutoCAD.DatabaseServices.BlockReference")
                            {
                                BlockReference thisBlockRef = (BlockReference)trans.GetObject(thisOid, OpenMode.ForRead);
                                arrList.Add(thisBlockRef.Name);
                                List<string> atrCol = new List<string>();
                                returnOIDs.Add(thisOid);
                                foreach (var atrRef in thisBlockRef.GetAttributes())
                                {
                                    atrCol.Add(atrRef.Tag);
                                    atrCol.Add(atrRef.TextString);
                                }
                            }
                        }
                        return (arrList);
        ...
        public void insertTagNumbering(Point3d ptInsert, string strBlockName, double dscale, string strLayerName, ArrayList strArrayList)
        {
            Database db = HostApplicationServices.WorkingDatabase;
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                BlockTable bt = (BlockTable)db.BlockTableId.GetObject(OpenMode.ForRead);
                BlockTableRecord btRec = (BlockTableRecord)tr.GetObject(bt[BlockTableRecord.ModelSpace], OpenMode.ForWrite);
                ...
                BlockTableRecord btr = (BlockTableRecord)bt[strBlockName].GetObject(OpenMode.ForRead);
                BlockReference bRef = new BlockReference(ptInsert, btr.ObjectId);
                btRec.AppendEntity(bRef);
                tr.AddNewlyCreatedDBObject(bRef, true);
                if (btr.Annotative == AnnotativeStates.True)
                {
                    ObjectContextManager ocm = db.ObjectContextManager;
                    ObjectContextCollection objColl = ocm.GetContextCollection("ACDB_ANNOTATIONSCALES");
                    Autodesk.AutoCAD.Internal.ObjectContexts.AddContext(bRef, objColl.CurrentContext);
                }
                int ccAttCounter = 0;
                foreach (ObjectId objid in btr)
                {
                    ...
                    if(ent is AttributeDefinition)
                    {
                        ...
                        if (strArrayList.Count -1 >= ccAttCounter)
                        {
                            attref.TextString = strArrayList[ccAttCounter].ToString();
                        }
                        else
                        {
                            attref.TextString = "";
        ...
        static public void CalcBlocks1()
        {
            ...
            SelectionFilter sf = new SelectionFilter(Tvp);
            PromptSelectionResult psr = ed.SelectAll(sf);
            SelectionSet sSet = psr.Value;
            ObjectId[] idArray = sSet.GetObjectIds();

            using (Transaction trans = db.TransactionManager.StartTransaction())
            {
                BlockTableRecord bTblRec = (BlockTableRecord)trans.GetObject(db.CurrentSpaceId, OpenMode.ForRead);
                foreach (ObjectId objTblId in bTblRec)
                {
                    if (bTblRec.HasAttributeDefinitions)
                    {
                        Entity et = (Entity)trans.GetObject(objTblId, OpenMode.ForRead);
                        BlockReference bRef = (BlockReference)trans.GetObject(objTblId, OpenMode.ForRead);
                        Entity et2 = (Entity)bRef;
        ...
        private static Editor acEd;

        public static List<string> GetBlokByName(SelectMode mode)
        {
            ...
            switch (mode)
            {
                case SelectMode.CurrentDwg:
                    using (Transaction trans = acdB.TransactionManager.StartTransaction())
                    {
                        BlockTable blkTbl = acdB.BlockTableId.GetObject(OpenMode.ForRead) as BlockTable;
                        foreach (ObjectId ObjTblId in blkTbl)
                        {
                            BlockTableRecord blkTblRec = (BlockTableRecord)ObjTblId.GetObject(OpenMode.ForRead);
                            if (blkTblRec.HasAttributeDefinitions)
                            {
                                newList.Add(blkTblRec.Name);
                            }
                        }
                    }
                    break;
                case SelectMode.PickFromDwg:
                    TypedValue[] tvs =
                    {
                        new TypedValue(0, "INSERT")
                    };
        ...
        public static List<string> GetBlokByName(string FileName)
        {
            List<string> newList = new List<string>();
            acdB = new Database(false, true);
            try
            {
                acdB.ReadDwgFile(FileName, System.IO.FileShare.Read, false, "");
            }
            catch 
            {
                AcAp.ShowAlertDialog("Error Read File Drawing");
            }
            using (Transaction trans = acdB.TransactionManager.StartTransaction())
            {
                BlockTable blkTbl = acdB.BlockTableId.GetObject(OpenMode.ForRead) as BlockTable;
